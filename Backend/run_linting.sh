#!/bin/bash
# Script pour exécuter les outils de linting et formatting via Docker

set -e

# Use docker-compose exec to run commands inside container
DOCKER_CMD="docker-compose exec -T backend python3 -m"
...
if $DOCKER_CMD black --check app/ tests/ 2>/dev/null; then
    echo -e "${GREEN}✓ Black: Code correctement formaté${NC}"
else
    echo -e "${YELLOW}⚠ Black: Certains fichiers nécessitent un formatage${NC}"
    echo -e "${YELLOW}  Exécutez: docker-compose exec backend python3 -m black app/ tests/${NC}"
    exit_code=1
fi
echo ""
...
if $DOCKER_CMD flake8 app/ tests/ 2>/dev/null; then
...
if $DOCKER_CMD isort --check-only app/ tests/ 2>/dev/null; then
    ...
    echo -e "${YELLOW}  Exécutez: docker-compose exec backend python3 -m isort app/ tests/${NC}"
    exit_code=1
fi
echo ""
...
if $DOCKER_CMD mypy app/ tasks.py 2>/dev/null; then
